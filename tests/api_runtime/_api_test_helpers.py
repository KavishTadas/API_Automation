"""Shared HTTP runtime for the API test tiers. AUTHORED -- edit this file.

This was emitted into ``tests/auto_generated/`` until Phase 3, which was wrong in
a way that only showed up when that directory was about to become disposable
build output: three authored modules import from here, one of them at module
level (``tests/global_contract/auth_bootstrap.py``). A library the authored tier
cannot import without is not generated output, whatever directory it sat in.

Consumers::

    tests/global_contract/auth_bootstrap.py   perform_api_request   (module-level)
    tests/global_contract/catalogue.py        load_runtime_config, _resolve_templates
    harness/service.py                        load_runtime_config
    build/auto_generated/*.py                 the generated tier

``perform_api_request`` holds the pooled ``httpx.Client`` from b5ee0d3. The
cookie jar is cleared before every send: a session cookie surviving between
requests would carry an authenticated identity into the checks that assert an
unauthenticated caller is rejected, and those would start passing for the wrong
reason.
"""
