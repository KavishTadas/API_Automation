"""Duplicate Policy Name Collision Defect

Imported from the attendance repo's `Duplicate Policy Name Collision Defect` probe (Attendance Policy), payload verbatim.
...
PROBE_BODY = json.dumps({'policyName': 'LeaveAttendance', 'policyHeading': 'Leave and Attendance Rule', 'policyDescription': 'Testing duplicate policy name creation without uniqueness gating.'})

#: What must already exist for the probe to mean anything.
REQUIRES = "a record this payload would collide with"


def test_duplicate_policy_name_collision_defect():
    pytest.skip(
