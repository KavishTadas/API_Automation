"""Duplicate Employee Assignment Collision

Imported from the attendance repo's `Duplicate Employee Assignment Collision` probe (Late-Early POLICY), payload verbatim.
...
PROBE_BODY = json.dumps({'policyCode': None, 'policyName': 'Duplicate Employee Collision Test 2027', 'description': 'Testing duplicate employee in same assignments array', 'templateMode': 'CUSTOM', 'eventCountMinutes': 30, 'graceMinutes': 10, 'graceEvent': 2, 'allowedEvent': 3, 'deductionType': 'LEAVE', 'leaveDeductDays': 0.5, 'leaveTypeId': 1, 'effectiveFrom': '2027-01-01', 'effectiveTo': '2027-12-31', 'isDefault': 'N', 'isActive': 'Y', 'assignments': [{'employeeId': 3585, 'effectiveFrom': '2027-01-01', 'effectiveTo': '2027-12-31', 'isActive': 'Y'}, {'employeeId': 3585, 'effectiveFrom': '2027-01-01', 'effectiveTo': '2027-12-31', 'isActive': 'Y'}]})


def test_duplicate_employee_assignment_collision_is_refused():
...
def test_duplicate_employee_assignment_collision_does_not_crash_the_server():
    response = probe_response(caseRef, PROBE_BODY)
    reached_handler(response)
