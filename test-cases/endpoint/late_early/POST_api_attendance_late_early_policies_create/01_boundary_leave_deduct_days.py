"""Boundary Leave Deduct Days

Imported from the attendance repo's `Boundary Leave Deduct Days` probe (Late-Early POLICY), payload verbatim.
...
PROBE_BODY = json.dumps({'policyCode': None, 'policyName': 'Extreme 100 Days Leave Deduction 2036', 'description': 'Testing missing upper limit on leaveDeductDays', 'templateMode': 'CUSTOM', 'eventCountMinutes': 30, 'graceMinutes': 10, 'graceEvent': 2, 'allowedEvent': 3, 'deductionType': 'LEAVE', 'leaveDeductDays': 100.0, 'leaveTypeId': 1, 'effectiveFrom': '2036-01-01', 'effectiveTo': '2036-12-31', 'isDefault': 'N', 'isActive': 'Y', 'assignments': [{'employeeId': 3496, 'effectiveFrom': '2036-01-01', 'effectiveTo': '2036-12-31', 'isActive': 'Y'}]})


def test_boundary_leave_deduct_days_is_refused():
...
def test_boundary_leave_deduct_days_does_not_crash_the_server():
    response = probe_response(caseRef, PROBE_BODY)
    reached_handler(response)
