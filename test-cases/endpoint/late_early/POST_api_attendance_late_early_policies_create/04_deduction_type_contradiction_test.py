"""Deduction Type Contradiction Test

Imported from the attendance repo's `Deduction Type Contradiction Test` probe (Late-Early POLICY), payload verbatim.
...
PROBE_BODY = json.dumps({'policyCode': None, 'policyName': 'Deduction Contradiction Test 2027', 'description': 'Testing LEAVE deduction with fineAmount populated', 'templateMode': 'CUSTOM', 'eventCountMinutes': 30, 'graceMinutes': 10, 'graceEvent': 2, 'allowedEvent': 3, 'deductionType': 'LEAVE', 'leaveDeductDays': 0.5, 'leaveTypeId': 1, 'fineAmount': 500.0, 'effectiveFrom': '2027-01-01', 'effectiveTo': '2027-12-31', 'isDefault': 'N', 'isActive': 'Y', 'assignments': [{'employeeId': 3585, 'effectiveFrom': '2027-01-01', 'effectiveTo': '2027-12-31', 'isActive': 'Y'}]})


def test_deduction_type_contradiction_test_is_refused():
...
def test_deduction_type_contradiction_test_does_not_crash_the_server():
    response = probe_response(caseRef, PROBE_BODY)
    reached_handler(response)
