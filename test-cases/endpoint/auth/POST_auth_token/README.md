...
Suite: `auth` · endpoint slug: `auth_post_auth_token`
...
```python
caseRef = "post|/auth/token|auth|login"
```
...
- **Login**
  - `caseRef`: `post|/auth/token|auth|login`
  - assertion state: `asserted-not-executed` — currently in `bruno/auth/login.bru`
