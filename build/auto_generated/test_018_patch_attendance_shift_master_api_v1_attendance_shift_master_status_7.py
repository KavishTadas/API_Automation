"""Generated tests for API row 18.

This file is generated by scripts/generate-generic-tests.py.
Do not edit by hand; update api-docs/API_File and regenerate.
"""

from __future__ import annotations

import json

import allure
import pytest

from tests.api_runtime._api_test_helpers import (
    assert_response_matches_schema,
    perform_api_request,
    skip_if_no_response_schema,
)


API = json.loads(r"""{
  "Sr. No": "18",
  "Module Name": "Attendance Shift Master",
  "Sub-Module Name": "Toggle Delete",
  "Access": "private",
  "Functional Purpose": "Toggle Delete",
  "Base URL": "{{baseURL}}",
  "Endpoint / Path": "/api/v1/attendance/shift/master/status/7",
  "HTTP Method": "PATCH",
  "Request Parameters": "query: action=DEACTIVATE | headers: Authorization=Bearer {{authToken}}",
  "Request Body Schema": "",
  "Response (example/200)": "",
  "Owner / Developer": "Parth Divekar <parthdivekar987@gmail.com>",
  "API Identifier": "patch|/api/v1/attendance/shift/master/status/7|attendance shift master|toggle delete",
  "Request Body": "",
  "Example Request Payload": "",
  "Example Response Payload": "",
  "Dependent APIs / Services": "Requires authToken from Employee Auth API; Depends on Employee Auth API POST /auth/token for authToken",
  "Comments": "Source: collections/Attendance_Management_API.json; No saved response example in source file"
}""")

@allure.title("Toggle Delete \u2014 returns HTTP 200")
@allure.epic("Attendance Shift Master")
@allure.feature("PATCH /api/v1/attendance/shift/master/status/7 — Toggle Delete")
@allure.story("HTTP Status Code Check (200)")
@allure.parent_suite("Attendance Shift Master")
@allure.suite("PATCH /api/v1/attendance/shift/master/status/7 — Toggle Delete")
@allure.sub_suite("HTTP Status Code Check (200)")
@allure.label("sourceModule", "Attendance Shift Master")
@allure.label("sourceType", "Python auto-generated")
@allure.label("owner", "Parth Divekar <parthdivekar987@gmail.com>")
@allure.link("patch|/api/v1/attendance/shift/master/status/7|attendance shift master|toggle delete", name="API Identifier")
def test_attendance_shift_master_api_v1_attendance_shift_master_status_7_status_code(api_runtime_config: dict[str, str]) -> None:
    allure.dynamic.parameter("HTTP Method", "PATCH")
    allure.dynamic.parameter("Endpoint Path", "/api/v1/attendance/shift/master/status/7")
    allure.dynamic.parameter("Expected Status", "200")
    response = perform_api_request(API, api_runtime_config)
    assert response.status_code == 200


@allure.title("Toggle Delete \u2014 response matches documented schema")
@allure.epic("Attendance Shift Master")
@allure.feature("PATCH /api/v1/attendance/shift/master/status/7 — Toggle Delete")
@allure.story("OpenAPI Schema Validation Check")
@allure.parent_suite("Attendance Shift Master")
@allure.suite("PATCH /api/v1/attendance/shift/master/status/7 — Toggle Delete")
@allure.sub_suite("OpenAPI Schema Validation Check")
@allure.label("sourceModule", "Attendance Shift Master")
@allure.label("sourceType", "Python auto-generated")
@allure.label("owner", "Parth Divekar <parthdivekar987@gmail.com>")
@allure.link("patch|/api/v1/attendance/shift/master/status/7|attendance shift master|toggle delete", name="API Identifier")
def test_attendance_shift_master_api_v1_attendance_shift_master_status_7_response_schema(api_runtime_config: dict[str, str]) -> None:
    allure.dynamic.parameter("HTTP Method", "PATCH")
    allure.dynamic.parameter("Endpoint Path", "/api/v1/attendance/shift/master/status/7")
    skip_if_no_response_schema(API)
    response = perform_api_request(API, api_runtime_config)
    assert response.status_code == 200
    assert_response_matches_schema(response, API)
